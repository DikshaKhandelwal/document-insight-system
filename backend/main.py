...
async def generate_audio(request: AudioRequest):
    ...
    #                 # Truncate merged segments to fit target length, but preserve alternation
    ...
    #                         break
    #                     take = min(len(text), allowed_chars - char_count)
    #                     if take > 0:
    #                         truncated_segments.append((speaker, text[:take]))
    #                         char_count += take
    #                 merged = truncated_segments
    ...
    #             # SSML validation and logging
    #             print("\n--- SSML to be synthesized ---\n")
    #             print(ssml_text)
    #             print("\n--- End SSML ---\n")
    #             # Basic validation: check for empty segments
    #             if '<voice' not in ssml_text or '</voice>' not in ssml_text or len(ssml_text) < 50:
    #                 print('SSML validation failed: missing voice tags or too short.')
    #                 raise ValueError('SSML validation failed: malformed or empty SSML.')
    ...
    #             # Do NOT set speech_synthesis_voice_name globally; let SSML control voices
    ...
    #     total_duration = sum(file.get("duration_estimate", 0) for file in audio_files)
        # Generate podcast script using Gemini LLM or fallback
        if LLM_AVAILABLE:
            insights_context = ""
            if request.insights:
                for insight_type, content in request.insights.items():
